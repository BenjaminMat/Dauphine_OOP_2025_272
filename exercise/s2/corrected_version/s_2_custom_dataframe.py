...
print("1D array:", arr1) ==> Output : 1D array: [1 2 3 4 5]

arr2 = np.array([[1, 2, 3], [4, 5, 6]]) # Create a 2D NumPy array (matrix)
print("2D array:\n", arr2)
==> output :
2D array:
 [[1 2 3]
...
# Access elements
print("Element at row 1, column 2:", arr2[0, 1]) ==> output : Element at row 1, column 2: 2
...
class DataFrameSimple:
    def __init__(self, data):
        ...
        data_in_cols_as_arrays = [np.asarray(data[col]) for col in self.columns]
        self.data = np.array(data_in_cols_as_arrays).T  # shape: (n_rows, n_cols)

    def _col_index(self, column_name):
        if column_name not in self.columns:
            raise KeyError(f"Column '{column_name}' not found")
        return self.columns.index(column_name)

    def mean(self, column_name):
        idx = self._col_index(column_name)
        return np.mean(self.data[:, idx]).item()

    def sum(self, column_name):
        idx = self._col_index(column_name)
        return np.sum(self.data[:, idx]).item()

    def min(self, column_name):
        idx = self._col_index(column_name)
        return np.min(self.data[:, idx]).item()

    def max(self, column_name):
        idx = self._col_index(column_name)
        return np.max(self.data[:, idx]).item()
